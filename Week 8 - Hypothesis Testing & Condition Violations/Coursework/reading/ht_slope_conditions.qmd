...
First, a **hypothesis** is a statement about the value of an unknown population parameter. In our example, our population parameter of interest is the slope of the relationship between baby's birth weight and mother's age for **every** baby born in the US in 2014. This parameter is notated with $\beta_1$.
...
Throughout this section, we'll use a pre-specified significance level $\alpha$ = 0.05 for our hypothesis tests. I'll leave the discussion on the choice of this $\alpha$ value until later on in @sec-errors. 
...
Observe that the resulting data frame has `r nrow(births_generate) %>% scales::comma()` rows. This is because we performed shuffles / permutations for each of the `r nrow(births)` rows `r n_reps` times and $`r (nrow(births)*n_reps) %>% scales::comma()` = `r n_reps` \cdot `r nrow(births)`$. If you explore the `births_generate` data frame, you would notice that the variable `replicate` indicates which resample each row belongs to. So it has the value `1` `r nrow(births)` times, the value `2` `r nrow(births)` times, all the way through to the value `r n_reps` `r nrow(births)` times. 
...
The fourth column of the regression table in @tab-regression, `statistic`, corresponds to a *test statistic* relating to the following *hypothesis test*:
...
Recall that you can intuitively think of the $p$-value as quantifying how "extreme" the observed fitted slope of $b_1$ = `r obs_slope` is in a "hypothesized universe" where there is no relationship between a baby's birth weight and a mother's age. 
...
In Figure @fig-non-equal-variance-residuals I present an example where the residuals clearly do not have equal variance across all values of the explanatory variable $x$.
