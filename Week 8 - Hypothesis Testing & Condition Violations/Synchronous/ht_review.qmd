...
__Step 2:__ `hypothesize()` what would happen under the null

. . .

__Step 3:__ `generate()` permuted resamples

. . .

__Step 4:__ `calculate()` the statistic of interest
...
- Why is the `hypothesize()` function used to make a null distribution but not 
for a bootstrap distribution?

- What does the `null = "independence"` input in `hypothesize()` mean? What is 
...
::: {style="font-size: 1.5em; color: #FFFFFF;"}
__Step 1:__ Calculate observed statistic

__Step 2:__ Find observed statistic on permutation distribution
:::
