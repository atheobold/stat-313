...
## The Variance of a Numerical Variable

We’ve previously studied two summary statistics of the *spread* (or *variation*) of a numerical variable: the standard deviation and the interquartile range (IQR). We now introduce a third summary statistic of spread: *the variance*. The variance is merely the standard deviation squared and it can be computed in `R` using the `var()` summary function within `summarize()`^[The formula for variance is the square of the standard deviation, or $\text{variance} = \text{sd}^2 = \frac{(x_1 - \bar{x})^2 + (x_2 - \bar{x})^2 + \cdots + (x_n - \bar{x})^2}{n - 1}$]
...
we can apply the `get_regression_points()` function^[Technically, the `get_regression_points()` function does the **same** thing as the `augment()` function you learned in the R tutorials last week! ] to our saved model, in this case `model_2_interaction`:

```{r interaction-augment}
#| echo: true
...
Let’s repeat the above calculations for the parallel slopes model and compare them in the table below. 

```{r parallel-variances}
#| echo: false
#| message: false

interaction_var <- get_regression_points(model_2_interaction) %>% 
  summarize(var_y = var(average_sat_math), 
            var_y_hat = var(average_sat_math_hat), 
            var_residual = var(residual), 
            r_squared = var_y_hat / var_y)

parallel_var <- get_regression_points(model_2_parallel_slopes) %>% 
  summarize(var_y = var(average_sat_math), 
            var_y_hat = var(average_sat_math_hat), 
            var_residual = var(residual), 
            r_squared = var_y_hat / var_y)
...
interaction_var |> 
  ...
  knitr::kable(
    caption = "Comparing variances from interaction and parallel slope models for MA school data"
    ) |>
  kableExtra::kable_styling()
```

Observe how the $R^2$ values are near identical at around 0.699 = 69.9%. In other words, the additional complexity of the interaction model only improves our $R^2$ value by a near zero amount. Thus, we are inclined to favor the “simpler” parallel slopes model.

### Extending to `evals` data

Now let’s repeat this $R^2$ comparison between interaction and parallel slopes model for our models of  
$y$ = teaching score for UT Austin professors. In the *ModernDive* textbook, you visually compared using parallel slopes or different slopes to model the relationship between `age` and teaching `score` for male and female professors. 
...
interaction_var <- get_regression_points(evals_interaction) %>% 
...
parallel_var <- get_regression_points(evals_parallel) %>% 
  summarize(var_y = var(score), 
            var_y_hat = var(score_hat), 
            var_residual = var(residual), 
            r_squared = var_y_hat / var_y)

interaction_var |> bind_rows(parallel_var) |> 
  knitr::kable(
    caption = "Comparing variances from interaction and parallel slope models for UT Austin evaluation scores data"
    ) |>
  kableExtra::kable_styling()
```
