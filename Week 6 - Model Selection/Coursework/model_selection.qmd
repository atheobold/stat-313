...
$$R^2_{adj} = 1 - \frac{\text{var(residuals)}}{\text{var}(y)} \times \frac{(n - k - 1)}{(n - 1)}$$
...
We start with the model that includes **no** predictors. Then we fit each of the possible models with just *one* predictor. Then we examine the $R^2$ for each of these models^[Remember, for a basic regression with only one variable, we use (unadjusted) $R^2$!]:
...
We repeat the process again, this time considering the adjusted $R^2$^[We've moved to adjusted $R^2$ because we now have more than one predictor variable!] for all 2-predictor models where one of the predictors is `term` and with a new baseline of $R^2 = 0.12855:$
