...
This data set has 32,735 observations (rows) of 16 variables (columns).
...
Yes.  Carrier, Tail Number, Origin, and Destination are all character type categorical variables.  They are strings.
...
The histograms have different bin widths. (haha, I don't know any other word for it)  They vary in specificity.  The smaller the bin width, the more accurate the distribution is in representing the true shape.  With a fatter bin width, the skew does not appear to be as abrupt.  The histogram with binwidth 15 has a 10x higher resolution than that with the 150, which reveals that the majority of flights are on time or very close to on time, with a long right tail.  
...
a.  We could have plotted a scatterplot of any two numerical variables; a barplot of different origins, destinations, or carriers; a boxplot to show the distribution of observations of any numerical variable; or just a histogram of any other numerical variable.

b.  Histograms are good at revealing the distribution shape of a set of data over only one variable.  It can be difficult to determine the mean and quartiles just from looking at the histogram, whereas a boxplot is very good at representing these values.

<div style="color:blue">
Your description of plotting options is correct, but I wanted you to focus specifically on the other plotting options for one numerical variable. Are there aspects of a distribution that you cannot see in a boxplot that you can in a histogram? What about a dotplot? What are its benefits / weaknesses? 
</div>
...
The distribution of arrival delay on flights from any New York airport to LAX is skewed to the right.  More than half of the flights arrive early, but the mean arrival is late.

```{r}
## Code for exercise 2 goes here! 
nycflights%>%
  filter(dest == "LAX")%>%
  ggplot(aes(arr_delay)) + geom_histogram()
nycflights%>%
  filter(dest == "LAX")%>%
...
<div style="color:blue">
Great visualization and data summaries! Are the summaries you found appropriate for this distribution? If a distribution is skewed, what is an appropriate measure of center / spread? 
</div>
...
dd_monthly <- aggregate(dep_delay ~ month, nycflights, mean) %>%
  arrange(desc(dep_delay))
dd_monthly$month <- as.factor(dd_monthly$month)
print(dd_monthly)
```

<div style="color:blue">
While your process works, it does not use the tools we have learned in this course. I am unfamiliar with the `aggregate()` function, so I cannot provide you with assistance with your code. Additionally, converting a interger variable to a factor doesn't result in a better table output, unless you specify what the order of the levels are.  
</div>
...
nycflights %>%
  ...
  xlim(-50,550) +                       # I lost a few extreme datapoints here, but its okay because it wont change
  geom_histogram(binwidth = 10)  +      # the shape of the rest of the grpah, which is the important part
  facet_wrap(~month)
```

<div style="color:blue">
Nice work faceting by month! Your axis selection removes observations from the dataset. Is this approriate? Why so? Technically, you do not need to change month to a factor before faceting, as `ggplot()` will facet the same if it is an integer. 
</div>
...
The visualization clearly shows the shape of the data over each month.  This gives us a better monthly understanding of the data compared to the summaries.  Also, this shows us that the general shape of the data (skewed right with one peak) is more or less the same throughout the year.
