...
This dataset has 32,735 rows with 16 columns.
...
<!-- Your response goes below this line! -->
Yes,Carrier, tailnumber, origin and destination are and have a data type of character.
...
In histograms that have bigger binwidth the shape becomes more noticable but we lose bars. In the bigger binwidth graphs we lose the first small bar of the histogram.

<div style="color:blue">
Nice start! What does having a bigger binwidth obscure in the histogram. Do you think there would be a binwidth that would be too small?   
</div>
...
<!-- Your response goes below this line! -->
a) A density plot or dot plot could of been used.
b) Histograms can be misleading especially if the binwidth is not set correctly. Histograms are also difficult to find the exact value of a point. A plus of histograms is the fact that they are useful when working with large ranges and make it easy to compare data

<div style="color:blue">
Great description of the strengths and weaknesses of histograms, but how does this relate to the other plotting options? Could we use a boxplot to visualize these data? What information would be included in that plot that is not in a histogram?  
</div>
...
## Not having a space between these two different code tasks makes me think they are connected! 
## I'd recommend adding a space between the different coding tasks in your R scripts. 
```
...
The distribution the histogram is unimodal with a right skew. The mean of the dat is 1.47 

<div style="color:blue">
Great visualization and data summaries! Are the summaries you found appropriate for this distribution? You mention a right skew, but then calculate the mean. Is this appropriate?   
</div>
...
<div style="color:blue">
While technically your process works, it results in **a lot** of variables that we don't really care about. What I wanted instead was for you to chain each of these steps together with the %>% operator. Make certain you can do this, and if you have questions ask! 
</div>
...
It seems like the data is fairly even over the months. The data is skewed to the right and unimodal. 
