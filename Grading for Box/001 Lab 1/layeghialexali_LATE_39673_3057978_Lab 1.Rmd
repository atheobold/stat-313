...
There are 32,725 rows and 16 columns in the nycfligths dataset.
...
int - Day
chr - Tail number
These are both categorical variables in the nycflights dataset.
...
All three histograms have a smilar skew. They are skewed right, meaning more data is falling on the left side of the histograms.One histogram is not as skewed as the others, it has less datapoints in the farthest left area. The other two histograms have the bulk of the data in the area farthest left.
...
A. Numerical variables can be visualized with a dot plot. Dot plots look very similar to histograms, except there is no filled in are to show the count. Dot plots use "dots" stacked on top one another to show a data point. Both histograms and dot plots are similar in showing numerical variables.

B. Histograms are strong in showing if the data is skewed or not. One weakness is histograms can lead someone to think differently due to the scale of the axis. 
...
```
There are 3,565 flights to SFO in February
...
The shape of the arrival of LAX flights is not normally distibuted. If this histogram was normally distibuted there would be a mountain shape in the center with each end sloping down. This data is somewhat skewed.
...
The visualization is a more complete picture than data summaries. In the data summaries you do not see the outliers. With the visualization all is seen. This is important to understand more about the distribution of the data not just the mean median and other important statistics.
