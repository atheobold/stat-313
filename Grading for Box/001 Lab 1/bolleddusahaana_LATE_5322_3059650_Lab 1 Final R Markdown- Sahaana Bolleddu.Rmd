...
<!-- Your response goes below this line! -->
The data was collected for flights that departed from NYC in 2013.
...
<!-- Your response goes below this line! -->
Yes, there are categorial variables in the dataset. Their names are: carrier (two letter character abbreviation), tailnum, origin (the airport abbreviation), and destination (the airport abbreviation).
...
The three histograms vary in their binwidths. We can see that smaller binwidths give us a more concise interpretation of the data points. As the binwidth becomes larger, we can see that the data is less precise and it becomes more difficult to analyze. Each histogram had a different binwidth, which altered the visualization of the data.
...
<!-- Your response goes below this line! -->
a. Some other visualizations that we could have produced are scatter plots and box plots.
b. Histograms are a simple way to visualize data, where none of the data overlaps. The data is presented in an organized manner, and the audience can easily interpret the data. An issue with scatter plots is that the dots can sit on top of one another, which can make it difficult to visualize overlapping data points. Some data may be overlooked or missed because it is harder to see. Box plots are useful for analyzing minuscule details, such as viewing outliers and it also gives a wider scope of the data to view.

<div style="color:blue">
How many variables are plotted in a scatterplot? How many variables are we interested in plotting? 
</div>
...
count(dest == "SFO")
```
68 flights were found heading to SFO in February.
...
nycflights %>%
group_by(month) %>%
summarise(mean_dd = mean(dep_delay), median_dd= median (dep_delay), iqr_dd = IQR(dep_delay), n_flights = n()) %>%
arrange(desc(mean_dd))
```

<div style="color:blue">
Nice work combining all of these pieces together! Woohoo! Data rodeo! 
Technically, you only needed to calculate one summary statistic, but good practice calculating others! 
</div>
...
nycflights %>%
group_by(month) %>%
summarise(mean(dep_delay), median_dd= median (dep_delay), iqr_dd = IQR(dep_delay), n_flights = n())
ggplot(nycflights, aes(x = factor(month), y = dep_delay)) +
geom_boxplot()
```

<div style="color:blue">
Sid-by-side boxplots are one option of visualizing different levels of a numerical variable, but you were asked to use faceting. 
</div>
...
From the visualization, the audience can gain an easier understanding of what they are looking at. Data summaries only utilize numbers, which is why it may be confusing and difficult for the audience to understand. The visualization clearly displays the 12 months and the distribution of departure delays, which is helpful when interpreting data. The box plot that was created in number 8 gives the audience an easy visual to follow along, instead of solely following numbers.
