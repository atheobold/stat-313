...
<!-- Your response goes below this line! -->
The categorical variables are year,month,day (integer), carrier (character), origin,dest (character)
...
The main difference is the amount of values on the x axis contained in each bar or bin.  By including a greater spread of values in each bin, like 150 in the second code, it can obscure some of the specifics of the data. The histogram from the first code shows there are far fewer flights with negative delays than with near-zero delays, with flights with near-zero delays making up a vast majority.  The nuance of this is lost in the binwidth = 150 histogram
...
<!-- Your response goes below this line! -->
a) Other visualizations of a single variable that could have been used are dotplots, density plots, and boxplots.
b) The nycflights dataset is very large, so while dotplots may be more beneficial for visualizing data in smaller datasets, at the scale we're working with, it would be a lot more difficult.  Density plots may lose some of the nuance of the data, particularly in the code where binwidth = 15.  Density plots would also give us a big-picture view, however, and is also good for when a large dataset is being used. Boxplots also plot summaries of the data, similar to a histogram, but they can be a bit more complicated and difficult to understand to a general audience.  For a dataset that has such a massive majority like the nycflights data with departure delay around the 0 minute mark, it may also provide less clear results than a histogram.
...
The mean arrival delay was 1.46, while the median arrival delay was -6.  The standard deviation was 39.4. The majority of values fall between -50 and 50, however, the minimum value was -62 and the maximum value was 319.  


```{r}
## Code for exercise 2 goes here! 
filter(nycflights, dest == "LAX")%>%
...
The visualization allows us to compare the values more easily.  The descending sorting of the data summary makes it a bit difficult to see the mean departure delays on a month-by-month basis, while the graph makes it easy to see.  The graph also helps us see if there seem to be any trends, for example, rising departure delays through April, high departure delays in June and July, and falling departure delays from August to September.
