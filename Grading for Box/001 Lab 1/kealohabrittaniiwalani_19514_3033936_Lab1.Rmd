...
<!-- Your response goes below this line! -->
  The data set 'nycflights' has 32,735 rows and 16 columns


1. (b) When were these data collected? 
<!-- Your response goes below this line! -->
  This data was collected from a random sample of flights that departed NYC (i.e. JFK,     LGA or EWR) in 2013.
...
<!-- Your answer goes below this line -- do not remove this!  -->
  the histograms are all showing the same data however the last one is the easiest to 
    understand because the span of graph is closest to the the data therefore the graph
    is big enough to understand
...
<!-- Your response goes below this line! -->
  ...
  a)scatterplot, box plot, line chart 
  
  b) a histogram in this case is simple and easy to read to show that majority of flights
      left on time. however the down side being that you can not see individual data points and their impact on the data, like you would see in a scatter plot 
...
the distribution seem slightly shewed right since the shape of the curve and the number of outliers in the histogram.
...
nycflights %>%
  group_by(month) %>%
  summarise(mean_delay = median(dep_delay)) %>% arrange(desc(mean_delay), month)
...
g1 <- ggplot(data = nycflights, mapping = aes(month,dep_delay), xaxt("s"))+
       geom_point()
month_labels <- list("Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul",
          "Aug", "Sep", "Oct", "Nov", "Dec" )
## changing the axis to clearly show the months 
g1 + scale_x_continuous(breaks = seq(1, 12, by = 1), 
                      labels = month_labels)
```
...
In the summary, it is hard to find trends this is easier in a visualization, also personally it is easier to spot outliers on a graph than in the summary 
