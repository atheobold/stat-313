...
the histogram with 15 as the binwidth, the data is divided into more columns,and provides more detailed information on a narrower time frame. It shows that most flight depart on time and then as the delay time becomes longer on the x-axis, the count on the y axis gradually decreased in a relatively smooth curve. The hisgram with 150 as the binwidth, on the other hand, divided the data in a larger chunk, and into only 4 visible columns. and it does not show the trend of as delayed time increase, the count gradually decrease. The graph shows that most flight depart with in 75 min before and after the original departure time. There appears to be just a sudden drop after 75 min of delay, the flight count becomes significantly less.And the count after 225min and 375 min become barely observable. Another thing is that since graph 2 (150 binwidth) combines more data into one column, the y-axis's interval(0-3000) is larger than the first graph's interval (0- 2000). In general, graph 1 shows a more detailed distribution than graph 2
...
a. Dotplot, Density plot, boxplot
b. -The benefits of a histogram include:
    1. compare to Density plot or box plot, it shows a clear view of the frequency distribution
    2. you can easily find the variation using histogram
    3. it also categorize the data that fall within a specified range of values (bins)
    4. Compare to the other 2 graph, it is easier to locate the min and max value.
  
   -The weakness of a histogram include:
   1. If we choose the inappropriate binwidth, it can be misleading. If we use too less block,some important information will be left out, and if we use too many, it might be hard to analyze.
   2. Unlike Density ploth, it will not show a continuous trend and the sense of the shape of the distribution can change depending on the bin width that is selected.
   3. Density plot can be easily layered to show more information but it's hard to layer histogram
   4. Histogram are based on only 2 sets of data for x and y axis, and sometime to see a full picture we might need more than 2 factors.
...
The distribution of the arrival delay is unimodal distribution and slightly right-skewed. The mean is at 1.46 min, median is at -6 min, standard deviation is 39, variance is 1553, and minimum delay is -62 min and maximum delay is 1583 min.
...
October has the highest number of flight that is departure roughly on time and February has the lowest amount of flight that departure on time. Moreover, the distribution of flight for every month is similar that all the graph is right skewed with one peak near 0 min delay.
