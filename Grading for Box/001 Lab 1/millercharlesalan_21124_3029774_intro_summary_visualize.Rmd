...
<!-- Your response goes below this line! -->
It appears this data was collected in 2013. 


1. (c) Are there categorical variables in the dataset? If so, what are their names and data types? 
<!-- Your response goes below this line! -->
Yes, there are categorical variables. We can quickly identify several, by observing the related data types. For example: 'origin', 'dest', and  'carrier' are categorical variables that are non-numeric. In addition, we have 'year', 'day', 'hour', and 'minute' for ordinal variables.
...
Generally, the three histograms display a similar trend - that the majority of delays fall well within 250 minutes. Notably, however, with the bin size of 15, we do see a trend absent in the other histograms. This being, that there are a sizable portion ~2500 or more flights that are only delayed 15ish minutes or less. 
...
<!-- Your response goes below this line! -->
a. We could've produced visualizations as a scatterplot or a dot plot. 

b. Scatterplots are most favorable when we're trying to display the relationship and trends between two variables. So in this case, it would not likely be preferable. A dot plot, like a scatterplot, would have the advantage of displaying a trend and representing the datum individually. However, a dot plot would still be unsuited for this example, because of the size of our dataset. There are tens of thousands of records in the dataset, and plotting them individually would be overly complex and likely disorienting. Meanwhile, a histogram has the benefit of grouping data and plotting them as general trends. For this reason, I believe the histogram is most appropriate. I do believe this particular histogram could benefit from having its data scaled/transformed to account for outliers that lead to a skew.
...
## I keep this code around, because it looks like the demo code further on breaks without it.
lax_flights <- nycflights %>%
  filter(dest == "LAX")
...
num_sfo_feb_flights <- sfo_feb_flights %>% count()
```
...
lax_flights %>% summarise(med_ad = median(arr_delay), 
...
In particular, I could not easily envision the shape of the data. Having the data visualized as histograms, I'm better able to obtain the trends of departure delays. Additionally, it's easier to differentiate how the data compares, by having them plotted side by side. 
