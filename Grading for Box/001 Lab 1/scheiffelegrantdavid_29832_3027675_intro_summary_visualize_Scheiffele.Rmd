...
<!-- Your response goes below this line! -->
month, day, carrier, origin and destination
...
Features are obscured within the binwidth 150 compared to binwidth 15. The data is split primarily into the first bin in this histogram, whereas the distribution is much more detailed within a bin width of 15.
...
<!-- Your response goes below this line! -->
A) A box and whisker plot would also be a viable option for analyzing this distrubution. A normal distribution curve could also have worked for this data set.

B) a histogram can suggest data skew and allows for comparison between different segments of data.The interval set with a histogram is also within our control for more detailed comparisons. A histogram, however, may not as readily reflect key statistical values such as the mean as a box plot might, or outlying variables which may be of interest. 
...
The distributiooin of arrival delays appears normal, with a mean of 1.46 hours and a median of -6 hours. 
##The mean arrival delay time iss 1.46 hours
## The median arrival delay time is -6 hours
## There are 1,583 observations.

```{r}
## Code for exercise 2 goes here! 
## To determine mean arrival delay time
lax_flights%>%
  summarize(mean_ad = mean(arr_delay), median_ad = median(arr_delay), n = n())
##Mean arrival delay time is 1.46 hours
...
11    11    6.10
12    10    5.88
July has the greatest mean delay departure time
...
desc_del %>%
arrange(desc(mean_dd), .by_group = TRUE)
```
...
dist_del <- nycflights %>%
  group_by(month)
ggplot(dist_del, aes(x=month))+geom_histogram()
...
While there is a large difference in the mean departure delay for new york city flights, across each month there appear to be a similar number of instances in delayed departures from new york city airport.The data summaries depict departure delays by month well, however this best reflects the consistent magnitude of delayed departing flights from new york city.
