...
The 'nycflights' dataset has 32,735 rows and 16 columns
...
It was collected in the year 2013 from January through December. 
...
carrier, tailnum, origin, dest, year, month, day, flight 

<div style="color:blue">
Nice work! Keep in mind that if these variables were labeled a `fct` that would *also* be a categorical variable! We'd like to think of month, day, and year as categorical, but what datatype did `R` assign them? 
</div>
...
In both the binwidth = 150 and 30 histograms, the negative departure delays (I am assuming this means the flight took off early?) are not visible as their own bin. Negative departure delays are an important part of this dataset, so obscuring this feature is not as informative of this variable. The binwidth = 15 shows the negative departure delays as assigned to their own summary bin below zero. 

<div style="color:blue">
Nice work! The smaller binwidth makes certain aspects of plot more visible, like the flights that departed early. Do you think there would be a binwidth that would be too small?   
</div>
...
(answer a.) We could have used a Dotplot to represent this numerical variable. We could have also used a density plot to visualize this data. 

(answer b.) A histogram is useful for observing distribution of data. Meaning, they show the shape of the data better than a Dotplot (right-skewed, biomdal, left-skewed, etc.). However, histograms have the disadvantage of not showing raw data observations. They only provide summaries of the observations in the form of bins along the x-axis with the appropriate variable of interest. Dotplots are useful in that they exhibit the individual raw observations on the plot itself. Therefore, it could be reporoduced given only the plot. A disadvantage of Dotplots is that they can get messy and cluttered with variables containing very high numbers of observations.This variable (dep_delay) is better represented by a histogram because it has 509,124 observations in it. A Dotplot would be too confusing and messy in this case. A density plot could have been used to represent this variable as well. However, I still believe a histogram would be more appropriate in the case. Density plots are used as smoothed version of a histogram. In this variable, there is a big spike that a density plot would not represent well. Density plots are subject to spiking easily. As well, the histogram represents the values below zero well where as a density plot would smooth this bin out. 


<div style="color:blue">
Great point about how the y-axis would be a mess with a dataset this size! What about a boxplot? What are the benefits / weaknesses of a boxplot? 
</div>
...
nycflights %>%
  ...
  geom_histogram() + labs(x = "Departure Delays") + theme_classic() + ggtitle("Departure Delays: LAX")
## Awesome use of a different theme and a plot title
## I would strongly recomment that you use a new line after every + sign, to increase code readability! 
```
...
nycflights %>% filter(month == 2, dest == "SFO") %>% count()
## I would highly recommend having a new line after every %>% sign, for code readability! 
...
The distribution of flight arrival delays to LAX is uni-modal, right-skewed. I chose these specific summary statistics because they are the most robust and least susceptible to outliers. Since the data is uni-modal, it made sense for me to calculate the median as most of the data is condensed in in/around the singular mode. However, there is some large outliers that would affect the mean if I were to use that over the median. I chose the IQR to measure the spread of the distribution because IQR would be least susceptible to changes from the large outliers. Standard deviation and variance would be altered more by these large outliers than IQR. The IQR is 30.5 and the median is -6. 
...
nycflights %>%
  ...
  geom_histogram(binwidth = 10) + labs(x = "Arrival Delays") + ggtitle( "Arrival Delays: LAX")
## See my previous comment about new lines. 
...
Which month would you expect to have the highest average delay departing from a NYC airport?
...
nycflights %>%
group_by(month) %>%
  summarise(mean_dd = mean(dep_delay)) %>% arrange(desc(mean_dd))
## Again, a new line after every %>% is great! 
```
...
ggplot(data = nycflights, aes(x = (dep_delay), fill = month)) + labs(x = "Departure Delays", y = "Density", fill = "Month") +
  geom_density(alpha = 1, bw =4) + facet_wrap(vars(month)) + xlim(-20, 200)

#I would really have liked to transform this data using the log() function, but doing so eliminates all of the negative observations (19,936 observations) from the dep_delay variable from being in the plots. So, I did not transform it using the log() function. I did make the trade off of using xlim to scale down the x-axis. I lost some of my largest outliers (270 observations), but this is far less observations than I would have lost if I chose to transform the variable using log(). If I were to transform it instead, I would use a density plot and my code would be:

#   nycflights$month <- as.factor(nycflights$month)
#   ggplot(data = nycflights, aes(x = log(dep_delay), fill = month)) + labs(x = "Departure Delays", y = "Density", fill = "Month") +
#   geom_density(alpha = 1) + facet_wrap(vars(month))

```

<div style="color:blue">
Nice work faceting by month! Why did you choose a density plot? Why do you think that the negative observations should be removed? Wouldn't the story about departure delays be inaccurate without them? What about flights with 0 delay. Those also cannot be log transformed.   
</div>
...
In this data visualization, I can mainly see the changes in departure delays by month. Some months like month 12 have a lower density of approximately 0 or less than 0 flight delays. However, months like 9 and 10 have a high density of flights with 0 or less flight delays. It is intuitive for months like 12, 6, and to have less flights without delays and more flights with delays because these are the prime travel times due to summer vacation and the December holidays. 
