...
It has 32,735 rows and 16 columns.
...
<!-- Your response goes below this line! -->
Yes, carrier (character), tail number (character), origin (character), and destination (character).
...
With a binwidth of 15 the bars in the histogram go by increments of 15 which shows more differences in count between different departure delays. The default binwidth of 30 is generally a good measure to see the overall shape of the distribution but doesn't give all the details. Where as if binwidth is equal to 150 the bars on the histogram are counting in increments of 150 departure delay minutes so you don't see any detail because the data is in 3 big groupings.
...
a. You could have also produced a box plot, dot plot, and density plot are all plots you can use for this data.

b. The benefits of a histogram are that we are able to see the bigger picture of the shape of the disrtibution. However not all data is suitable to be presented in a histogram; histograms use a numerical variable on the x-axis and a count on the y-axis. Also we have the ability to change the bin width of the graph which if too small can give us too much information and make it harder to see the overall trend.  If too big then we miss out on important details that get lost with fewer and bigger bars. A box plot will show you where the median, Q1, and Q3 lie in the data but dont show the overall trend of the data (its shape) or any mode(s) in the data. A dot plot will show you what data repeated the most and similar summary data as a box plot but if you have a lot of data it will not be an effective graph choice. A density plot puts a curve on a histogram distribution and shows a dataset's overall trend but is sensitive to the spikiness of the data so it is better suited for larger datasets. 
...
## Not having a blank line between these two tasks makes me think they are connected. 
## I would recommend adding a blank line between the different tasks in your code. 
```
...
The distribution of this graph is multimodal with three peaks: One at zero, 1000, and 2000. The mean of the distribution is 1530, the median is 1554, the IQR is 864, the standard deviation is 575, and the sample size is 1583.

<div style="color:blue">
Great visualization and data summaries! The nultiple modes seems to be a relic of the binwidth chosen by default. How would you improve the plot? 
...
ggplot(data = lax_flights, aes(x=arr_time))+
  geom_histogram(binwidth=50)

lax_flights %>%
  summarise(mean = mean(arr_time),
            median(arr_time),
            iqr = IQR(arr_time),
            sd(arr_time),
            n())
...
<div style="color:blue">
You don't need to paste the output here, it is automatically output from R. 
</div>
...
ggplot(nycflights, aes(x = log(dep_delay))) +
           geom_histogram() +
...
<div style="color:blue">
Nice work faceting by month! Is the log appropriate here? How many observations does it remove? How does this change the inference we can make about this relationship? 
</div>
...
The visualization shows that for all months there is a similar distribution of delay times. Which we can see a lot faster than we could just looking at the departure delay data and the means. We can also see that the majority of delays each month peak at 2-4 minutes with distributions that are mostly unimodal but there are some bimodal graphs.
