...
Yes, there are categorical variables in the dataset. The names of the categorical variables include carrier, tailnum, origin, and dest which are all <chr> or "character" variables.
...
More detail is shown in the second histogram with banwidth of 15. The departure delay times is more accurate and shows that there are times other than 0 with counts below 5000. The banwidth of 150 is very obscure and does not show much detail. 
...
We could have produced boxplots, density plots, dotplots, or numerical summaries. Histograms are useful when comparing a large amount of data/observations because we ccan put them into bins. They are similar to dotplots, but for a larger amount of data which makes it preferred over dotplots. They are convenient for understanding the shape of the distribution and whether the data is skewed. In addition, they are great for identifying modes (the prominent peak in the distribution). The weaknesses of a histogram include that the data presented can be misleading. Too many bins could make for a difficult analysis whereas too few bins could leave out key details in the data. In addition, hard to display multiple different comparisons.
...
ggplot(data = nycflights, aes(x = dep_delay, y = factor(month))) +
  geom_boxplot()
...
The visualization makes it easeir to visualize the distribution of the departure delay times across each month. We are able to see the mean average times for each month along with the minimum and maximum values which would have been harder to see and compare from the data summaries. The graph gives us a clear and easy visual reference of the data.
