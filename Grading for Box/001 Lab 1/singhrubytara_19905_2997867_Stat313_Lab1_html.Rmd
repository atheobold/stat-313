...
<!-- Your response goes below this line! -->
Yes, there are categorical variables. Carrier and origin destination are nominal, and tail number, and flight number are ordinal.
...
The histogram with the smallest binwidth has the most detailed info, and the one with the biggest binwidth has less detailed data.
...
dot plot, density plot, boxplot
(b) What are the benefits and weaknesses of a histogram compared to these other options? 
<!-- Your response goes below this line! -->
It is easy to view and compare the data with a histogram, and they can be used with large ranges of info. A weakness of a histogram is that they can be misleading sometimes, because using too many blocks can make it hard to read but using too few blocks result in data loss. Density plots show a smoother representation of the data, but they're best used when you have a large number of observationss. Boxplots show only summaries of the data and are best used when you need to compare more than one distribution at once, and to figure out the outliers. However, they can't tell you when a distribtuin has more than one mode.
