...
<!-- Your response goes below this line! -->
Yes, year-numerical, month-numerical, day-numerical, carrier-character , origin-character, destination-character.
...
Because the histograms depict the same data but with varying bin widths, each graph displays different counts, based on the size of each bin. The second and third graphs are most distinct from eachother, as their values for binwidth are the most different from eachother. The second graph (the one with lowest bin width), displays counts for relaitvely narrow portions of data (bin width=15), meaning it tends to be more accurate, while the third histogram sports a very high bin width (150), meaning the count values will be represented with larger bars, meaning counts for departure delay are based on a much larger range of data. Finally, the first graph sports a bin width in between that of the second and third histogrms, meaning the potential for counts based on dep_delay in the first graph falls in between the first two. Overall, varying bin widths determine the magnitude of grouping of data points, which thus affects the value of the count. 
...
<!-- Your response goes below this line! -->
Histograms allow one to visualize frequency distribution and are highly versatile.However, as viewed above, histograms for the same data can appear vastly different based on bin width values. A weakness of histograms, then, is that the way the data is displayed may be disleading depending on bin width size. Not only this, but it seems that histograms may not be a valid option when comparing the impact of multiple variables or when graphing the relationship of categorical variables.
...
Allows one to easily visualize the distribution of means for each month that is not apparent when viewing the original data set.  
