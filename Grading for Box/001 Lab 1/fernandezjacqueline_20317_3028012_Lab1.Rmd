...
<!-- Your response goes below this line! -->
The data in this set was collected in 2013.
...
<!-- Your response goes below this line! -->
There are categorical variables in this dataset. These variables include "carrier", "tailnum", "origin", and "dest", and they are all characters.
...
In the histogram that uses the bin width of 15, we can notice that there is a lower frequency of delays slightly below zero, on the left side of the mode. In the other histograms, it appears that there a mode around zero, but it obscures this smaller frequency.
...
<!-- Your response goes below this line! -->
We could have also used a dotplot, boxplot, or density plot. A histogram is beneficial because it can show data distributions and modes. However, histograms do not plot raw data, unlike some of the other plots such as the dotplot. Additionally, density plots also show approximations of data distributions, but are very sensitive to data fluctuations. Finally, a boxplot is also a data summary, but it doesn't allow for analysis of distributions and modes. 
...
The distribution of this histogram is right-skewed and unimodal. 
...
lax_flights %>%
  summarise(mode_ad = mode(arr_delay),
            mean_ad = mean(arr_delay),
            median_ad = median(arr_delay))
```

<div style="color:blue">
Great visualization and data summaries! Are the summaries you found appropriate for this distribution? Why so? 
</div>
...
From the visualization, we can tell that each month has a right-skewed, unimodal data distribution.
