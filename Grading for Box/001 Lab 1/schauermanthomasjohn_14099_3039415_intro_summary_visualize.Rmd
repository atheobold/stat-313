...
There are 32,735 rows and 16 columns in the 'nycflight' dataset.


1. (b) When were these data collected? 
<!-- Your response goes below this line! -->
The data were collected throughout the year of 2013.


1. (c) Are there categorical variables in the dataset? If so, what are their names and data types? 
<!-- Your response goes below this line! -->
Year- categorical ordinal. Month- categorical ordinal. Carrier- categorical.  Tailnum- categorical. Flight- categorical. Origin- categorial. Destination: categorical.                                                                                                         
...
As you decrease the binwidth, more bars appear on the graph so we can observe more specific measurements on typical delay times. Smaller bars increase the amount of information we can interpret from the graph. Larger bars provide us with more general observations about a majority of the data.
...
<!-- Your response goes below this line! -->
a) A box plot, dot plot, and density plot are other visualizations that we could have produced.
b) The benefits of a box plot are observation of the median, IQR, and outliers of a dataset. This provides us with specific statistical information not shown on a histogram. However, box plots don't show multiple modalities as well as a histogram or other plot. Density plots are good at showing two different modes. Dot plots are hybrids of box plots and histograms as you can more easily see individual data points which is helpful for giving an idea of IQR, but you also see mode as the number of dots stacked up on a specific value on the x axis.  
...
The data is skewed right with more outliers and extreme observations comprised of longer arrival delays. The maximum is 319 minutes, with the mean at 1.46 and the median at -6. The median tells us that over half of the observations were early arrivals by at least 6 minutes. The standard deviation of 39.41 tells us around 68% of the data falls within that much of the mean, so the maximum is a very extreme outlier. 
...
nycflights %>%
  group_by(month) %>%
  summarise(mean_dd = mean(dep_delay), median_dd = median(dep_delay), n_flights = n()) %>%
...
ggplot(data = nycflights, aes(x = dep_delay)) + geom_boxplot() + facet_wrap(vars(month))
library(ggridges)
ggplot(data = nycflights, aes(x = dep_delay, y = as.factor(month), fill = as.factor(month))) + geom_density_ridges() + labs(y = "Month", x = "Departure Delay (min)") + theme(legend.position = "hide")
...
We can see that June and July have the largest IQRs on the boxplot, as well as June has more outlier extreme delays than July even though July has a higher mean delay. It also appears that April has a smaller IQR than March, however April also has a higher mean delay time than March. January also has the highest departure delay outier yet the second smallest mean delay. 
