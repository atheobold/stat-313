...
<!-- Your response goes below this line! -->
'carrier', 'tailnum', 'origin', 'dest' are all the categorical variables in this dataset stored as character data. 
<!-- I believe that the date columns 'year', 'month', and 'day' are each both ordinal and numerical data, and are stored as integers. I am uncertain how to think of dates as variable types.  -->

<div style="color:blue">
Nice work! Keep in mind that if these variables were labeled a `fct` that would *also* be a categorical variable! Your speculations about year, month, and day being numerical variables is spot on. We could use these variables to construct a "date" variable that would be continuous from January 1 to December 31, which would be an honest way to represent dates as a continuous quantity. 
</div>
...
The histogram using a binwidth of 15 shows the distribution greatest detail. In the larger bin sizes we miss the detail which shows a high frequency of on-time departures, some early departures, and the extent of the right skewed distribution of flight delay.
...
<!-- Your response goes below this line! -->
This same data could have been expressed in a ridge plot with origin (or other categorical variable) splitting the ridges. An additional alternative would be a density plot.

The weakness of a histogram, like the one we coded, is a lack of additional dimension and information. A ridge plot could compare delay times as it compares to destination or origin which adds an additional layer of information to compare visually. Similar arguments can be made for overlaying density plots. In actuality, a histogram could be modified to overlay multiple categorical variables (I have done it) and thus the only true weakness is that histograms are not very visually appealing to me compared to density plots. 

<div style="color:blue">
Great description of the benefits of adding a categorical variable to the histograms! But, why would we choose a histogram over a boxplot? I agree that density plots look nicer, but would one be appropriate in this situation? Why so?  
</div>
...
lax_flights %>%
  summarize(mean_dd   = mean(arr_delay), 
...
From the visualization you can get a sense of the overall distribution of delay for each month and the count of the most common bin. But other than that, there is not much you can gain from the visualization that you couldn't gain from the data summaries, and more than likely you could gain more from data summaries if we had extracted more than just the mean. 
