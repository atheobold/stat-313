...
There are 32,375 rows and 16 columns.
...
Month <int>, day <int>, carrier <chr>, tailnum <chr>, origin <chr>, destination <chr>

<div style="color:blue">
Nice work! Keep in mind that if these variables were labeled a `fct` that would *also* be a categorical variable! While we would like to think of month, day, and year as categorical, currently they are not assigned a categorical datatype. What datatype are they assigned?   
</div>
...
Both of the histograms depict data in the form of bins and map the height of the bar to the number of cases in the bin. The second histogram is more clear, with the bins visible and understandable to read. The first histogram shows narrowed bins and is much more difficult to interpret. 

<div style="color:blue">
Nice work! What about the third histogram? Is that more clear than the second? What makes the second histogram easier to read? 
</div>
...
Other visualizations that we could have produced (but are not limited to) include dot plots, density plots, or box plots! Due to the bins of a histogram, you cannot perfectly reconstruct the data set. This is when something like a dot plot would be more applicable.

<div style="color:blue">
Great description of a weakness of a histogram, but you didn't discuss the benefits / weaknesses of boxplots. Is a histogram the "best" choice? Why or why not? 
</div>
...
## The blank space between the two processes is a bit awkward. I would delete it! 
```
...
Skewed distribution to the right, and characterizes as unimodal.The data shows that the frequency of longer arrival delays is less common.The mean is revealed as 1.4 that considers an average of all of the data, but the median represents -6, consistent with the frequencies of early arrivals and 0 minutes of wait time that are prevalent at the far left of the distribution. 
...
nycflights %>%
  ...
  summarise(mean_ad = mean(arr_delay),
            ...
            n_flights =n())
## A line separating the summaries from the plot would be nice! 
```

<div style="color:blue">
Nice work! Great visualization and data summaries! What summary statistics are appropriate for a skewed distribution? 
</div>
...
The visualizations are able to depict the frequencies of specific data through a physical distribution, which data summaries do not provide. For many, a visual breakdown of the data is more easily understood, and can help with making inferences of the data that is provided.
