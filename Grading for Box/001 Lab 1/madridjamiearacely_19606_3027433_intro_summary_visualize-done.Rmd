...
<!-- Your response goes below this line! -->
The data is all from flights that occurred in 2013.


1. (c) Are there categorical variables in the dataset? If so, what are their names and data types? 
<!-- Your response goes below this line! -->
There are categorical variables. Carrier, destination, and origin are all character types.
...
The smaller the binwidth, the more detail we get to see.
However, the smallest one (bin= 15) is too zoomed out and we can't distinguish the peaks very well, but we can see the data is skewed.
With the middle one, we see a better distribution and recognize the peaks more. 
And the largest binwidth groups data in less bins which does not allow us to see good distribution. 
This is not great because we no longer see the trends, shape, and skews.
The changes in binwidth change our view of the shape, center, and distribution which then affect how we will describe the data.
...
We could use a density plot since it gives a more obvious representation of the histogram by using a smooth line.
They show the distribution shape a lot better compared to histograms.
We could also use a dotplot to see the distribution of a variable.
In this case however, a histogram is better because it shows the distribution without the need of showing each individual case.
A dotplot is nice because you know exactly how many observations are in the bin but it may not be the best when there is a large amount of observations for the variable.
Boxplots are also a good way to visualize numerical data.The benefit a histogram has is showing the distribution, but box plots are better for comparisons.
Boxplots are also good for looking at subtle skews in the data, which aren't always apparent in histograms.
Both however are good for large amounts of data. 
...
The arrival delays are skewed to the right.


```{r}
 ggplot(data = lax_flights, mapping = aes(x = arr_delay)) + 
 geom_histogram()
...
ggplot(data = nycflights,aes(y = dep_delay, x = factor(month))) +
geom_boxplot()
...
The side by side comparisons allow us to compare the distributions for each month, which we can't see clearly in summaries.
It also gives us an idea of many statistics like median, quartiles, and outliers, which are very informative and sometimes lack in summaries.
The trends are easier to spot than in the summary. 
