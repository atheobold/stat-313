...
Histogram with binwidth 150 has both the largest count and dep_delay, the binwidth 15 is the only one with data to the left of the most common answer
...
<!-- Your response goes below this line! -->
we could have produced a scatter plot or a pie chart. the histogram works as a better summary than the scatter plot, which enables viewing specific data points. the pie chart gives us percentages of each response but doesn't accurately summarize the distribution
...
filter((dest = sfo_feb_flights))
count((dest = sfo_feb_flights))
```
...
It is a fairly standard distribution with less differentiation left of the mean than to the right. The mean is 9.78.
...
ggplot(data = nycflights, mapping = aes(x = month, y = dep_delay)) +
geom_point()
...
You can better see the distributions of the data, along with trends being more apparent.
