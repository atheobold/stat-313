...
Yes there are categorical variables in the dataset, they include the characters 'carrier,' 'tailnum,' 'origin,' and 'dest.'
...
Each histogram utilizes a different binwidth. The first histogram has a binwidth of 30, the second has a binwidth of 15, and the last has a binwidth of 150. The shape of the individual data distribution is visualized best in the second histogram with the smallest binwidth, as it shows lots of variation in the bin heights. The count of the bar in negative departure delay, and its variation from the count at zero, is obscured in the histograms with binwidths or 30 and 150. In the plot with the binwidth of 150, there is no way to distinguish between any of the data values as there are only three bins displayed. 
...
A box plot or a dot plot could have been used to visualize the distribution of the data. A histogram does not plot raw data, it plots summaries of the data. So, while this is helpful in gaining a big picture of the shape of the distribution, it isn't great for looking at individual observations. In dot plots, each point represents a single case. Histograms are also helpful in identifying the mode of the dataset, while boxplots are unable to do so. 

<div style="color:blue">
Great description of the components and benefits of the different plotting options! What are the weaknesses of a dotplot? 
</div>
...
The distribution of the arrival delays of the LAX flights in the histogram is right skewed and unimodal. The mode lies around -10 minutes, so it appears that flights departing NYC and heading to LAX typically arrive a little early. Because the histogram is right skewed, a mean would not be an appropriate statistic to utilize because it's heavily influenced by extreme values. The median, however, is a robust statistic. The median of the arrival delays of the LAX flights is -6 minutes, and represents the middle number between -62 minutes (the minimum) and 319 minutes (the maximum). There were 1583 flights from NYC to LAX total in 2013.
...
From the visualization, you can look at the overall distribution and notice any patterns in the data. It's convenient for understanding the shape of the data distribution and identifying skew and prominent peaks. The mode is clearly visible in the visualization, and other popular values with high counts can be observed, too.
