...
The nycflights dataset is 32735 rows and 16 columns. 


1. (b) When were these data collected? 
<!-- Your response goes below this line! -->
The data were collected in 2013 over a 12 month period. 


1. (c) Are there categorical variables in the dataset? If so, what are their names and data types? 
<!-- Your response goes below this line! -->
The categorical variables are tailnum, origin, carrier, and dest.  These are all nominal data. 
...
When the bin width is smaller, more detail about the distribution of the data can be seen. We are able to observe this in the first fifteen or so observations, where as when the bin width is 150 we only see that most of the observations are in the first 150. 
...
<!-- Your response goes below this line! -->
A box plot could have been used. A box plot would allow us to see the distribution of data through quartiles, while the histogram we have now only shows us the distribution in the bins.  We could have also used a density  plot which would allow us to see the true density pf the observations, while the histogram only gives us the density within the bins. 
...
The data is slightly right skewed as the median is -6. This makes sense, as more flights are delayed than early. The mean is 1.46494, which is closer to 0 than the median, showing the effect of the high outliers shown. These outliers skew the mean but not the median, as it is more resistent to outliers.
...
I am able to observe the outliers present in the first and last month, and also that on average the months all look relatively similar excluding said outliers. The data visualization allows me to see which months had less delays and which had more, as well as make generalizations about the departure delay times in general. 
