...
The 'nycflights' dataset has 32,735 rows and 16 columns.This dataset contains information on 32,735 flights and has 16 variables.
...
<!-- Your response goes below this line! -->
These data were collected throughout the year 2013.
...
<!-- Your response goes below this line! -->
There are categorical variables in the dataset.They are year (integer), month (integer), day (integer), departure time (integer), carrier (character), tail number (character), flight (integer), origin (character) and destination (character). It is worth noting that R classifies year, month, day, departure time and flight as integers even though they are categorical variables. This is because these variables come in numerical form and R thinks that they are numerical variables.

<div style="color:blue">
Nice work! Keep in mind that if these variables were labeled a `fct` that would *also* be a categorical variable! Nice job describing the variables that should be treated as categorical, but were imported as numerical!    
</div>
...
The histogram with a binwidth of 150 makes it seem like almost all of the flights leave nearly on time. The histogram with a binwidth of 30 shows that while most flights leave on time, there are a portion of flights that leave late. Finally, the histogram with a binwidth of 15 shows that while most flights leave on time, there are some that leave early and some that leave late. All of the histograms show that a majority of flights leave on time. However, the histogram with a binwidth of 150 obscures the fact that there are a good amount of flights that are delayed. The histogram with a binwidth of 30 does a better job at showing these flights, but it doesn't accurately show how late these flights were as the histogram with a binwidth of 15 does. Finally, the histogram with a binwidth of 15 distinguishes the flights that left early and the flights that left on time. This fact is obscured in the other two histograms as one bar encapsulates both the early flights and the on-time flights.

<div style="color:blue">
Nice description! The smaller binwidth makes certain aspects of plot more visible. Do you think there would be a binwidth that would be too small?   
</div>
...
<!-- Your response goes below this line! -->
(a) Some other visualizations that we could have produced are (colored) density plots, box plots, dot plots and ridge plots

(b) One benefit of a histogram is that we can easily see some of the trends in the data. For example, we can easily point out that most of the flights leave on time. Another benefit of a histogram is that it uses bins, making the graph much easier to look at. Instead of having a dot for each of the 32,735 flights in the data set, the histogram puts each flight in a bin with flights of a similar delay time. However, this is also a weakness, as the histogram does not use raw data. Thus, some trends can get lost if the correct binwidth is not used. Another weakness is that histograms cannot use color to emphasize points while other graphs such as density plots can.

<div style="color:blue">
Great description of the components and benefits of a histogram! Technically, you can fill the color of your histogram to be whatever color you want! What are the benefits and weaknesses of density plots and boxplots compared to histograms?  
</div>
...
The distribution of the arrival delays of the LAX flights is approximately unimodal but it has a slight right skew.The median arrival delay is -6 and the IQR of the distribution is 30.5
...
ggplot(data = lax_flights, aes(x = arr_delay)) +
  geom_histogram(binwidth = 15)  
## I'd add a blank line between the code for the plot and the code for the summaries! 
...
The visualization shows that most of the flights from every month leave on time. The data summaries do not show that. This is because some months have more delayed flights and are more right skewed, causing the mean departure delay to increase. The visualization, however, shows a clear peak at around 0 for each month.
