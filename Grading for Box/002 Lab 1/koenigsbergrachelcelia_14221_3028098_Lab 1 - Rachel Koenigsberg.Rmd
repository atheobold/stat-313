...
month(integer), day(integer), carrier(character), tailnum(character), flight(integer), origin(character), dest(character)
...
When binwidth is lower, the variability of the data is revealed, and hidden when binwidth increases. The third histogram loses this information by having too large of a binwidth and clumping all of the values together.
...
Boxplot, dotplot, density plot

(b) What are the benefits and weaknesses of a histogram compared to these other options? 
<!-- Your response goes below this line! -->

A histogram does a better job of showing the number of cases in each bin with the height of each bin, showing the distribution of the data better. However, it is easy to lose important information with histograms by having bins that are too large, and analysis can be difficult when they are too small.
...
Of 1583 flights, the mean arrival delay was 1.47 minutes and the median was -6 minutes. The maximum delay was 319 minutes, and the minimum was -62 minutes, with an IQR of 30.5 minutes. OVerall, the data was right-skewed and unimodal.
...
ggplot(data = nycflights, aes(x = log(dep_delay))) +
  geom_density() +
...
Variability of the data, outliers, and shape
