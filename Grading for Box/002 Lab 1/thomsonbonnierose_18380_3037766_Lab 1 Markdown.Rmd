...
The nycflights dataset is very large, with 32, 735 rows (observations) and 16 columns (variables).

<div style="color:blue">
Correct! This is actually not that large of a dataset!  
</div>
...
The categorical variables are the airline carrier, origin, and destination. These are all regular categorical variables, they are not ordinal.
...
When the binwidth is set to 15, it is much easier to see that the vast majority of flights do depart on time. When the binwidth is not specified, R uses bins=30. This plot shows a strong right skewed histogram, but almost seems to suggest that a lot of flights leave early. When binwidth is set to 150, the histogram only shows 3 card, once again, suggesting that the largest bar that straddles 0 means that a lot of flights leave early. When set to 15, the histogram is easiest to read. It is still right skewed, but better shows the range of flights that are early, on time, or late. 
...
a. Another option for visualizing these data would be a scatterplot. It could also be shown on a dotplot, but the sample is so large that a histogram makes a lot more sense. 
b. Histograms are good for showing data density- for example, it is easy to see that most flights depart on time, few depart early, and some depart late. It is easy to identify the mode. They also are excellent for displaying the shape of data distribution- this plot is right skewed, but some may be a normal distribution or left skewed. Histograms are very useful for large data sets. However, histograms do not allow you to see each individual observation. As this lab showed, if bins are not set to a proper width, the histogram can become useless and not give any of the useful information discussed. 

<div style="color:blue">
Great description of the components and benefits of a histogram, but how does this relate to the other plotting options? What are the benefits / weaknesses of a dotplot? What other plots could be created for one numerical variable?  
</div>
...
ggplot(data = lax_flights, aes(x = dep_delay)) +
  geom_histogram()
## The blank line is a bit awkward. I would delete it! 
```
...
The distribution for arrival delays of LAX flights is right skewed. The mean is 1.465 minutes, meaning flights were only delayed by 1.465 minutes on average. The median is 6 minutes. Arrival delays ranged from -62 minutes to 319 minutes. 
...
lax_flights %>%
  summary(mean(arr_delay))

```

<div style="color:blue">
Great visualization and data summary! You reference the median, but it isn't calculated in your code. 
Are the summaries you found appropriate for this distribution? If a distribution is skewed, what is an appropriate measure of center / spread? 
</div>
...
nycflights %>%
  ...
  summarise(mean_dd = mean(dep_delay), 
            desc(mean_dd)) 
 ggplot(data = nycflights, aes(y = dep_delay, x = month)) +
  geom_point()
  
```

<div style="color:blue">
Did you inspect your plot? You are on the right track, but these points don't allow for us to see the **distribution** of the delays. Moreover, you did not facet by month. 

Why is the code for calculating data summaries included when you were asked to make a plot? 
...
The visualization gives a visual summary of the mean for each month's departure delays.It allows you to easily see which months have a much higher or lower departure delay mean than others, and easily draw comparisons and conclusions.  It also makes it easy to see outliers, which may alert you to issues in the data being skewed. 
