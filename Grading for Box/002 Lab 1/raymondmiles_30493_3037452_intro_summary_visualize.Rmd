...
<div style="color:blue">
You've got half of the information! How many rows in the dataset is there?  
</div>
...
<!-- Your response goes below this line! -->
Year, Carrier, Tail Number, Origin, Destination, Flight, 

<div style="color:blue">
Nice work! Keep in mind that if these variables were labeled a `fct` that would *also* be a categorical variable! Take a look at the year variable again, was it labeled as a categorical variable or as a numerical variable?   
</div>
...
Histograms with binwidth=30 and binwidth=15 show the skew of the data much more than binwidth=150. The histogram with a binwidth=15 shows that there are more flights that are delayed by a few minutes than are early by a few minutes. 
...
<div style="color:blue">
Because you provided no response for this question it is scored as a 0.  
...
The distribution of the arrival delays of flights at LAX ariport is unimodal and skews to the right. Arrival delays average around 1:45 minutes, however the median is -6 minutes. This shows the affect of the extreme cases. 

<div style="color:blue">
Nice work! Great visualization and data summaries! See my comments in your code. 
Excellent noting the impact of the outliers on the difference between the mean and the median! 
</div>
...
## I'd recommend using a new line (return after every ,) for each data summary. 
```
...
<div style="color:blue">
Nice work faceting by month! I would encourage you, however, to use `facet_wrap()` when faceting by only one variable. Why did you take the log? How many observations did that remove from the dataset? Is that appropriate? How does it change the inference we can make?  
</div>
...
The data summaries just told me what the average delay was for each month. The visualization gives a better measure of the spread. Many of these departure delay spreads are multimodal. They have a mode of 0 and 
