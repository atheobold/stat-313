...
Yes: month (int), day (int), carrier (chr), tailnum (chr), origin (chr), dest (chr)
where int ==  integer, and chr == character.

Integer variables are month and day which can be looked at as categorical.
Year was not included wince there is only 1 year (2013).

<div style="color:blue">
Nice work! Keep in mind that if these variables were labeled a `fct` that would *also* be a categorical variable! Good inclination to use month and day as categorical even though they were imported as integers!   
</div>
...
While all histograms show a skewed right distribution, the histogram with binwith = 15 shows the mode is not the lowest value as implied by the wide bins of histogram 1 and 3.
...
a) A dotplot, density plot, or boxplot could have been used (or variations of these plots).

b) A histogram only displays a summary of the data similar to a density plot or boxplot. A histogram is helpful when identifying the mode of data. A dotplot will look very similar to the histogram but will display data much more similar to the raw data (depending on binwidth). The density plot and boxplot do not accurately display count but the boxplot shows a measure of center, range, and potential outliers.

<div style="color:blue">
Great description of the components of the different plotting options! What are the weaknesses of a dotplot? Why don't density plots and boxplot accurately display the data? 
</div>
...
The arrival delays of LAX flights is a skewed right distribution with a median of -6 and IQR of 30.5. The negative value of the median represents an early arrival time. 50% of the data falls within a 30.5 minute range.
...
nycflights %>%
  ...
  labs(x = 'Departure Delay (min)', caption = str_wrap("Figure 1. A series of boxplots of Departure Delay for flights out of NYC sepapated by month (indicated on right)")) +
  theme(axis.text.y=element_blank(),
        axis.ticks.y=element_blank(),
        plot.caption = element_text(hjust = 0))
```
<div style="color:blue">
Nice work faceting by month and adding specifications to the plot. I would encourage you to stick with `facet_wrap()` for faceting by a single variable. If you want to specify rows / columns, there are `nrow` and `ncol` arguments that you can specify. 

There are a few things I would push you to think about: 
- Why a boxplot over a histogram or density plot? 
- What does faceting add that side-by-side boxplots wouldn't accomplish? 
</div>
...
Since I chose a Boxplot median, IQR, and outliers can be seen. In my opinion the key thing here is the outliers. There are quite a few outliers (one almost as far out as 1500 min).
