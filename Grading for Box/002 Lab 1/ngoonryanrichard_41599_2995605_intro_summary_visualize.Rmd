...
My data set is 32,735 rows by 16 columns large.
...
<!-- Your response goes below this line! -->
the data appears to be collected in 2013 according to the documentation.

<div style="color:blue">
Correct! Nice job inspecting the help file! Did you see if these were **all** of the flights out of NYC?  
</div>


1. (c) Are there categorical variables in the dataset? If so, what are their names and data types? 
<!-- Your response goes below this line! -->
There are categorical variables in the data set. They include: 
tailnum   chr 
origin    chr 
dest      chr

These are also variables that are numerical but tend to act like categorical variables
flight    int
hour      dbl 
minute    dbl
year      int 
...
<div style="color:blue">
Nice work! Keep in mind that if these variables were labeled a `fct` that would *also* be a categorical variable! Great job describing the variables that were impored as numerical but behave like categorical variables. Ideally, we would use the year, month, day, hour, and minute variables to make one "date" variable containing all of that information. Then, that variable could be honestly considered to be continuous!   
</div>
...
Compared to each other, they all have the same general shape. They all however reveal slightly different data. In each of the charts, the bins max out at different counts, with the lowest being around a count of 20000 for the smallest bin size and a count of 30000 for the larger bin size. One feature that is revealed is that is obscured in another is the sudden drop (that looks like an inverse root) in the count when you compare the chart with the smallest bin size to the one with the largest bin size.

<div style="color:blue">
Nice description! Where is this sudden drop? A more detailed description would be great! 
</div>
...
a) We could have made a boxplot and density chart.
b) histograms are better at showing underlying probility and the general shape of the data. Box plots are better at comparing multiple at sets and is better at showing of the skewness of a data (by easily revealing outliers).

<div style="color:blue">
What do you mean by boxplots can "compare multiple sets"? How is this different from a histogram? Can a histogram not show skewness or outliers? What are the weaknesses of boxplots or density plots? What about dotplots? 
</div>
...
<div style="color:blue">
Nice work, you are almost there! Your use of the `desc()` function is not getting you what you want. What needs to be arranged in descending order? 
</div>
...
ggplot(data = nycflights, aes(x = dep_delay)) +
  geom_histogram(binwidth = 100)+facet_grid(cols=vars(month))


```
<div style="color:blue">
Nice work faceting by month! I would encourage you to use `facet_wrap()` when faceting by one variable. 
</div>
...
I can see that the departure delay varies a bit by month.
