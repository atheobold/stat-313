...
This dataset has 32,735 rows and 16 columns.
...
Yes, they are year, month, day, departure time, arrival time, and flight number which are integer data and carrier, tail number, origin, and destination which are character data.

<div style="color:blue">
Nice work! Keep in mind that if these variables were labeled a `fct` that would *also* be a categorical variable! While we might want to treat month, day, and year as categorical, they were not imported as that datatype. What datatype are they? How would you change them to categorical variables?   
</div>
...
All three histograms are unimodal with an obvious spike near the left side of the data. The histograms with the 2 smaller binwidths show an obvious, longer right tail which suggests possible outliers and is more obscured in the histogram with the binwidth of 150.   

<div style="color:blue">
Nice description of the features that are evident in the smaller binwidth plots! The smaller binwidth makes certain aspects of plot more visible. Do you think there would be a binwidth that would be too small?   
</div>
...
We could have also produced a dotplot, boxplot, or a density plot. A histogram makes more sense than a dotplot in this case since there are a very large number of data points and dotplot displays raw data and are therefore best for small data sets. A boxplot would allow us to better see possible outliers and the center of the data than a histogram, but we can see the overall shape of the data better in the histograms. A density plot would also be good for visualizing the shape of the data but not great for identifying potential outliers.  
...
## Not having a blank space between these two tasks makes me think they are connected. 
...
This distribution is unimodal with a spike in the left side of the data and is skewed to the right. The median of this data is -6 and the IQR is 30.5.

<div style="color:blue">
Nice work! Great visualization and data summaries! 
...
You can see the shape of the distributions, their skew, how spread out they are, and you can better see if there may be potential outliers because you can see a representation of all of the data rather than just a single summary. 
