...
The dataset has 32735 rows and 16 columns. 
...
Yes, there are many categorical variables. Carrier, tailnumber, origin, and destination are regular categorical variables (character)
...
The binwidth is different between each histogram. The second histogram has the smallest binwidth and the third one has the largest. A feature only revealed on the second histogram is that there are numbers of flight depart earlier than its departure time.This is not showed in the other two histograms due to the binwidth.
...
a) Bar graph, Scatter plot, and boxplot
b) The advantage of histogram is simplicity and clear. It is easy to make and even people who do not have any background can understand and compare the frequency for each bar. Also, bars are connecting to each other which makes the comparison easier. The disadvantage of histogram is that it can not show an exact value since data are clustering together.

<div style="color:blue">
Great description of the strengths and weaknesses of a histogram! What are the stengths & weaknesses of the other plotting options? Also, use caution, what type of variable does a bar graph represent?  
</div>
...
<div style="color:blue">
You are very close! Your summary table shows the number of flights for each carrier in the dataset, but I wanted the **total** number of flights. You only need to change one part of your code to get this!  
</div>
...
The distribution of the arrival delays of the LAX flight is a right screwed histogram. Flights have more arrival delays than early arrivals.

<div style="color:blue">
Great visualization, but where are your summary statistics describing this distribution?  
</div>
...
<div style="color:blue">
You modified the RMarkdown file such that this is no longer a code chunk. Please don't do that, it could cause you knitting errors! 
</div>
...
By using the scatter plot, I can easily identify the gap between each month.I can clearly see the pattern of departure delay and able to make further interpretation; in this case, season can be a factor influencing the departure time.
