...
<!-- Your response goes below this line! -->
Yes, there is categorical variables in this data set. The names are year, carrier, flight number, origin, destination. With carrier, flight number, origin, and destination being nominal. And Year being ordinal.

<div style="color:blue">
Nice work! Keep in mind that if these variables were labeled a `fct` that would *also* be a categorical variable! Was `year` imported as a categorical variable? What is its datatyepe?   
</div>
...
The histograms are all displaying the same set of data but they are more or less detailed depending on their established binwidth or lack there of. The third graph has the largest binwidth and therefore reveals the least amount of data.  

<div style="color:blue">
You outlined that the larger binwidth plot reveals less, but you didn't say **why**. What features of the distribution are easy / hard to see for certain binwiths over others? 
</div>
...
<!-- Your response goes below this line! -->
Other visualizations that could be produced are dot plots, box plots, and mosaic plots. 
The benefits of histograms are that the data is easily readable and that it displays a wide range of data within one visualization, which can be harder to determine using other visualizations such as dot plots. One con of using a histogram is that width or scale of the histogram can influence the shape of the data and can skew actual data. 

<div style="color:blue">
Great description of the strengths and weaknesses of histograms! How do these relate to the other visualizations that could have been made? What is a "mosaic plot"? Is it used for a single numerical variable?  
</div>
...
From the summary statistics the median arrival delay is negative 6 which reveals that the middle quantity of delayed arrival actually isn't a delay and that the flights are usually early. I can also see that the mean arrival delay is 1.465, which makes sense because from the histogram there are a few large outlier's towards 200 and 300 which will skew the data. 

<div style="color:blue">
Great visualization, but where is the code for your data summaries? What summary statistics are appropriate for this distribution? 
</div>
...
From this visualization we can see the differences in delay times dependent on months. It allows us to easily view how delay times change from month to month, which makes it easier for us to analyze data. 
