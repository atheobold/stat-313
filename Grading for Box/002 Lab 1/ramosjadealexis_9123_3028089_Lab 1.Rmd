...
<!-- Your response goes below this line! -->
Carrier: chr, Origin: chr, Dest: chr, 
...
All three histograms share similar shapes. However, the histogram with a binwidth of 150 is much better because it shows the number of flights who did not have a departure delay was a bit above 30,000. It also shows the relative counts with more accuracy for flights that did have a departure delay.

<div style="color:blue">
Nice description! Are aspects of the distribution more difficult to see with a binwidth of 150? How does third histogram display counts more accurately, since they sample size in each plot is the same?  
</div>
...
<!-- Your response goes below this line! -->
(a)We could have produced a dotplot, boxplot, or Density.
(b) A histogram shows the distribution shape and shows the modes of the variable. Much like density and boxplot however, you cannot see the raw data like in a dotplot. The downside of a dotplot is that the y-axis is not useful.

<div style="color:blue">
Great description of the differences between the plotting options! What are the benefits / weaknesses of a boxplot or a density plot?  
</div>
...
## For this exercise, you need to change the code below! 
  ...
  nycflights %>%
    filter(dest == "LAX")
## You need to connect these filtered data to the ggplot below! 
  ggplot(data = lax_flights, aes(x = dep_delay)) +
      geom_histogram() 
```
...
<div style="color:blue">
You're on the right track, but the output from this code shows 1 not 68. What is the issue? What needs to be connecting `filter()` with `count()`? 
</div>
...
Only about 1.5 LAX flights had arrival delay, a maximum of 319 flights were delayed, and a total of 1583 flights were delayed.

<div style="color:blue">
Does the mean tell you *how many* flights were delayed? What are you taking the mean of? What are the units? 
Where is your description of the distribution? Are your summary measures appropriate, given the distribution? 
</div>
...
nycflights %>%
  group_by(month)%>%
  summarise(avg_dd  = mean(dep_delay)) %>%
  arrange(desc(avg_dd))
...
The outliers of delayed departures for each month and which months had the highest departure delays.
