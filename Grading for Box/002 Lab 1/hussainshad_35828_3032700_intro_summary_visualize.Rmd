...
The greater the binwidth, the more that the data becomes obscured. When the binwidth is decreased, it becomes easier to see just how many flights were delayed for a more specific amount of time.
...
<!-- Your response goes below this line! -->
Other options are boxplots, scatterplots, densityplots and/or dotplots.
The benefit to histograms is that it allows the viewer to see the general shape of the distribution, making it easier to draw general conclusions about the spread of the data. One weakness is that when given a massive set of data (such as this one), it can be hard to get the exact numbers, such as how many flights were delayed by a certain amount of time. If you wanted this information, it might be better to use another type of visualization that can more easily present that info. 
...
The majority of flights arrived at or around the scheduled time at LAX. Most delays were within 100 minutes of the expected arrival time. The mean delay time was by 1.46 minutes and median delay time was actually -6 minutes, meaning the flight arrived early.
```{r}
...
I was able to find the outliers of departure delays in each month whereas the data summaries only showed average delay times. 
