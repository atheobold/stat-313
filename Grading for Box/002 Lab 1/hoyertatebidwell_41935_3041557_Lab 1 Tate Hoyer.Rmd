...
With smaller binwidth, more detail can be seen that would be clumped together with a larger binwidth.
...
<!-- Your response goes below this line! -->
We could have produced boxplot. The advantage of the boxplot is that it gives a clearly defined median, quartile range and outliers but the advantage of the histogram is that it is easier to see the shape of the spread even if you can't necessarily see the exact median.
...
The distribution for arrival delays of LAX flights looks like a mostly normal distribution but with a slight positive skew given the amount of outliers on the right side. The mean arrival delay is 1.46 minutes and the median arrival delay is -6 minutes. The standard deviation is 39.4 minutes. 
...
lax_flights %>% 
  summarise(mean(arr_delay),
            median(arr_delay),
            sd(arr_delay))
  ggplot(data = lax_flights, aes(arr_delay)) + geom_histogram()
```
...
delays_by_month %>%
  ggplot(data = delays_by_month, mapping = aes(x = month, y = mean_dd)) + geom_col()
...
You can see the trends that occur over several months much better than just a list of the months with the highest means.
