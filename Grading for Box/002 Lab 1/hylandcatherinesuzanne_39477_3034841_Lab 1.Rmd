...
This data was collected during 2013.


1. (c) Are there categorical variables in the dataset? If so, what are their names and data types? 
<!-- Your response goes below this line! -->

There are categorical variables in the dataset.The categorical variables are year, month, day, carrier, tailnum, flight, origin, and dest. All of the above categorical variables are only categorical and not ordinal.
...
The three histograms are mildly similar, but they each have unique aspects. For the histogram without the binwidth, the graph shows a majority of the departure delay times data is right around zero. For the histogram with the binwidth of 15, the graph has more bars with larger proportions, creating more variability. For the histogram with the binwidth of 150, the bars are a lot wider, which create less variability with the departure delay data.
...
Other visualizations that we could have used include box plots or scatterplots.
The benefits of the histogram are: 1) it's easy to read and interpret, and 2) it can show the frequency of something occuring. However, a boxplot is better for showing values (i.e., means, medians, mins, and maxes), and a scatterplot is better for comparing specific values to find any relationships in the data.
...
Based on the histogram, it appears that the majority of the arrival delay data are around the 0 time interval, suggesting that a majority of the flights arrive on time or a little early/behind. Based on the summary statistics, the average arrival delay is about 1.5 minutes, while the standard deviation for the data is 39.413 minutes. The summary statistics, specifically the mean arrival delay time, suggests that the majority of flights are minimally delayed. 
...
ggplot(lax_flights, aes(x= arr_delay)) + geom_histogram(binwidth = 15)
```
...
mm <- nycflights %>%
  group_by(month) %>%
  summarise(mm = mean(dep_delay))
ggplot(mm, aes(x = month, y = mm, color=mm)) + geom_point()
...
The visualization shows that certain months (i.e., June and July) had larger average departure delay compared to other months (i.e., September-November). By looking at this visualization, I can see that the months with the larger average departure delay also happen to be busier months for airlines (i.e., the summer and winter months due to time off and holidays). By looking at just the data summaries, I could only see numbers rather than patterns.
