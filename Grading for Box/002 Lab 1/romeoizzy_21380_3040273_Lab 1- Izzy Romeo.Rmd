...
Yes, there are categorical variables in this dataset, specifically characters (chr). The categorical variables include carrier, tailnum, origin, and dest. 
...
In the histogram with the binwidth of 15, the mode is more clearly shown at about 0 departure delay. In the default binwidth and 150 binwidth histograms, this mode appears more spread out. Comparing all three together, it appears that the smaller binwidth corresponds to more details on the relationship between departure delay and count. While the binwidth of 150 shows less details, it appears less jagged. This type of graph may be favored by the NYC airport to show passengers/potential ticket buyers that they have little delays. The other histograms appear more jagged, but reveal more details. 
...
Other visualizations that could have been created for a numerical variable are dotplots. boxplots, and density plots. The benefits of histograms it that modes can be identified, the shape of the distribution is clear to see, and summaries (counts) of the data are plotted. A weakness of histograms is that no raw data is seen on the graph. Only data summaries are available. Compared to the other options, histograms do not clearly show outliers like boxplots, raw data like dotplots, and cannot overlay data like in density plots. While these weaknesses might be more significant depending on what a person hopes to obtain from a graph, histograms provide a great way to see data distribution and modes.

<div style="color:blue">
Great description of the components and benefits of the different plotting options! Can histograms not show outliers? 
</div>
...
<div style="color:blue">
Nice work! If you used just a "plain" `count()` at the end, it would only output the 68 (because it is counting the rows in the dataset). Why does your RMarkdown not display this output? What in your code is suppressing this output?  
</div>
...
The distribution of the arrival delays appears right-skewed when looking at the graph. Because of this, using IQR is a good way to see what numbers falls outside of the majority. In other words, outliers are highlighted. With the Q1 being -20 and Q3 being 10.5, it is easier to see that the arrival delays anywhere from 100-300 are outside of the normal. 

<div style="color:blue">
Nice work! Great visualization and data summaries! Excellent choice of data summaries based on the skewed distribution of arrival delays! What would be an appropriate measure of the center of the distribution? 
</div>
...
With data summaries, we are unable to see the overall distribution of the departure delays per month. Data summaries tell us what month exactly was the highest in regards to departure delays, but it doesn't show a summary of that month. The data visualizations also allow us to see what months have a wider range of departure delays compared to others. 
