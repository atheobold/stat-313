...
The `nycflights` dataset has 32,735 rows (observations), and 16 columns  (variables). 

<div style="color:blue">
Correct! Nice specification of what the rows represent!  
</div>


1. (b) When were these data collected? 
<!-- Your response goes below this line! -->
These data were collected in 2013, through a random sample of flights that departed NYC. 

<div style="color:blue">
Correct! Nice job inspecting the help file, now you know this was a random sample!  
</div>
...
<!-- Your response goes below this line! -->
Yes, there are categorical variables in the dataset. The categorical variables are carrier (data type = character), plane tail number (data type = character), flight number (data type = character), and origin and destination (data types = character). 
...
These three histograms all show that the histogram is unimodal, meaning there is one hump in all of the plots. All three histograms show that there is one big hump, and then more data shown by shorter humps to the right. However, one feature that is revealed only in the histogram with 15 bins is that to the left of the large mode (hump), there is one smaller hump, which is not evident in the histograms with bins of 30 or 150. In addition, the histograms with a smaller bindwidth display the plot with finer detail, so the histogram with a bindwidth of 15 has the greatest detail, while the histogram with a bindwidth of 150 does not have a great amount of detail and fine precision. 
...
<!-- Your response goes below this line! -->
a) Other visualizations that we could have produced are density plot, box plot, or dot plot. 
b) Compared to a density plot, a benefit is it is slightly easier to see the modes, but a weakness compared to the density plot is that it is a little more difficult on the eyes and is not as simple. Compared to a box plot, a benefit is that it is easier to see the spread of the data, but a weakness is that it does not show helpful data like the IQR. Compared to the dot plot, a benefit is that it is easier and nicer to look at since it is a big dataset and it is not as jumbled, but a weakness is that not every data point is represented as is on a dot plot. 

<div style="color:blue">
Great description of the components and benefits of the different plotting options! I like your pro / con analysis of each plot! 
</div>
...
nycflights %>%
  ...
  count()
```
The number of flights in this dataset that were headed to SFO in February is 68. 
...
The histogram in unimodal, meaning there is one mode, or one hump. The histogram has a right skew, and there are numerous outliers, which has an effect on the mean, and standard deviation. Since the shape of the histogram is unimodal and has a right skew, there are many outliers on the right (positive) side of the plot that can influence summary statistics such as mean and standard deviation. As we can see in the summary statistics, there is a sizable difference between the mean and median, and between the standard deviation and IQR due to the right skew which allows certain summary statistics (such as the mean and standard deviation) to be influenced by these outliers. As shown in the summary statistics, the mean arrival delay is roughly 1.465, whereas the median arrival delay is roughly -6. Similarly, the standard deviation of arrival delays is 39.413, whereas the IQR of arrival delays is 30.5. 

<div style="color:blue">
Nice work! Great visualization and data summaries! See my comments in your code. 
Excellent discussion of the difference in the data summaries based on the skewed distribution of arrival delays! 
</div>
...
<div style="color:blue">
Nice work faceting by month! Why did you choose a density plot? 
</div>
...
We can obtain that the plot is a unimodal density plot with a right skew, indicating that there are outliers to the right (positive) side of the graph. In addition, since density plots utilize proportions, and not the count, or departure delays, we have a better idea of the monthly departure delays seen with proportions compared to each other that we could not have seen from the data summaries. Because of this, we can see that at the mode/ hump of months 6 (June), 7 (July), and 12 (December), they have less height, which means they have less density at those heights, which is why they are more swayed by the outliers, which results in their higher means compared to the other months. Contrarily, we can see that months 9 (September), 10 (October), and 11 (November) have the highest density/ proportion at their peaks, which is why they are less swayed by their outliers, which results in them having the lowest means in this particular summary statistcs and histogram. 
