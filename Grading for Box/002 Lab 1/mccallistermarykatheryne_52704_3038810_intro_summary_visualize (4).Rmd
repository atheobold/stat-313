...
Your RMarkdown file cannot knit because of errors in your code for #4. You were expected to submit an RMarkdown file that successfully knitted, and are thus receiving a 10% deduction. 
...
<div style="color:blue">
You've got the numbers correct, but someone else might not understand what you mean when you say "__ by __ ". Which number corresponds to the rows and which corresponds to the columns?  
</div>
...
There are categorical values in the data set. Origin,Destination. These are both nomial because they do not have any natural order.  
...
In these three histograms we can see that the data falls heavily in the first part of the chart. This is common throughout the three. In the third histogram we can see that we get a zoomed in version of the second data plot due to the increase in bandwidth. I this is a little obscure because the count goes all the way to 30000. 

<div style="color:blue">
Nice description of where the data falls in the distribution, but it is not clear to me why a large count makes the plot more obscure. Are the features of the distribution that are easy / hard to see for different binwidths?
</div>
...
<!-- Your response goes below this line! -->
a. We could have produced a scatter polt 
b.The weaknesses of a histogram is the fact that we cant see each point of data we plotted. 

<div style="color:blue">
Yes, a histogram doesn't show the raw data, but what plot does? What are the strengths of a histogram? What are the other visualizations for **one** numerical variable? 
</div>
...
## For this exercise, you need to change the code below! 
 ...
 lax_flights <- nycflights %>%
## Notice that you aren't filtering the data like you should be
## And you aren't piping the data into the ggplot() function, like you should be 
+ ggplot(aes(data = lax_flights, x = dep_delay)) +
## Why does this line start with a + sign?   
+     geom_histogram()
## Why does this line start with a + sign?
```

<div style="color:blue">
Your code doesn't connect the data filtering process with the ggplot process with the pipe operator. Moreover, your code has errors which inhibit your RMarkdown file from knitting.  
...
## Not having a space between these two lines makes me think they are part of the same process. 
## I would encourage you to add a blank line between the different tasks in your code. 
```
...
<div style="color:blue">
Nice work! Great visualization and data summaries! Are the summaries you found appropriate for this distribution? Where is your description of the distribution? 
</div>
...
<div style="color:blue">
Did you visually inspect your plot? There is only one point for each month, corresponding to the mean. Here, we want to visualize the **distribution** of the departure delays faceted by month. </div>
...
From the visual data I can conclude that the winter months had the highest amounts of delays and that the delays slightly increase throughout the year with December being the month with the most delays. 
