...
The dataset has 32 rows and 16 columns 
...
<!-- Your response goes below this line! -->
orgin, destination. tailnum, carrier
...
All are skewed to the right. However, the histograms with smaller bins better show the curve, while larger bins turn the distribution into blockier, less defined skews. 
...
<!-- Your response goes below this line! -->
We could have used a dotplot 
b) Histograms allow us to see trends about dep_delay and how frequent it occurs in a given bin length. However bin length is very important as too large can leave out trends and too small can make analysis more difficult. 
...
ggplot(data = nycflights %>%
  filter(dest == "SFO", month == 2), aes(x = dep_delay)) +
  geom_histogram()
...
The histogram is skewed right with a median of -6 and a mean delay of 1:46. The data peaks around  slightly below zero meaning most flights were on time or early, however because of some outliers that pull the tail to the right the mean is much higher than the median. The median being -6 means half of the flights were early, which the mean does not reflect due to the skew. 
...
The visualization allows you to see outliers and how they affect the average. Data summaries include this in their average, which can lead to misleading statisitics. Visualization allows you to see what data points could be potential outliers and factor that into your analysis. 
