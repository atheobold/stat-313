...
Your R Markdown file will not knit because you have an error in your code for #4. Because you submitted a file that does not knit, you are receiving a 10% deduction. 
...
There are 32,735 rows and 16 columns in this dataset. 
...
There are categorical variables in the dataset. These are carrier, tailnumber, origin, and destination. 
...
The binwidth effects the thickness of each independent variable on the x axis. The first histogram (furthest above) had medium sized bins while the second histogram (in the middle) has very thin bins, and bottom histogram has thicker bins. This histogram only has three bins while the far left histogram has many more (~12 bins) as a result of the binwidth size differences. 

<div style="color:blue">
Nice description of the changes in the plots, but you didn't describe how these changes impact what the distribution looks like. What features are revealed / obscured by different binwidths? 
</div>
...
We could have used a dot plot or a boxplot to visualize the given data. A histogram can clearly show a shape of distribution unlike boxplots. Box plots allow us to see the median where histograms don't. Also, histograms are great for large samples unlike dot plots; instead, dotplots show raw data, which can be helpful to visualize data from smaller samples. The bins can furthermore customize data, making it easier to read with larger amounts of data. 
...
lax_flights <- nycflights %>%
  filter(dest == "LAX") %>%
  ggplot(aes(data = lax_flights, x = dep_delay)) +
  ## Your file will not knit because you have an error here. 
  ## You are piping the data into the plot, but then also specifying the data = argument, which confuses R. 
  geom_histogram()
```
...
Based on the histogram of arrival delays of LAX flights, the median of those particular flights are -1. It has an average of 1.46 flights that delayed upon arrival. 
...
nycflights %>%
  ...
  summarise(mean_dd = mean(dep_delay), 
            n_flights = n()) %>%
  arrange(desc(mean_dd)) %>%
  ggplot(aes(x = mean_dd, y = month)) +
geom_point() +
  facet_wrap(vars(month)) 
```
<div style="color:blue">
Did you inspect your plot? Each of the facets has a single point -- representing the mean for that month. Does that help us understand the **distribution** of departure delays? 
</div>
...
I was able to obtain a more precise and specific mean departure delay from the data summary because it puts the data in exact numeric values that were listed. In contrast, the visualization allowed me to see certain trends and patterns because it graphed the data and arranged it where you can look at all of them at once since they were set up next to each other. For instance, I noticed that during the beginning months, the departure delays were relatively low, but as the year progresses, it gradually increases.    
