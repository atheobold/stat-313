...
For this data set, each *bservation* is a single flight.
...
The categorical variables of the dataset are carrier, tailnum, origin, and dest (destination). For all the categorical variables, the data type for all the categorical variables in this dataset is character. 
...
The histograms all illustrate that the departive delays are relatively close to 0 minutes. In the histogram where the binwidth is adjusted very high, it loses details, such as close to 0 minutes being the most common departure time.
...
Other visualizations that could have been used includes a boxplot, density plot, and dotplot. The benefits of a histogram compared to these options is that a histogram can allow for easy comparison of data and allows for grouping of equal ranges of data together. In addition, histograms provide for a big picture of the shape of the distribution. However, its weakness is that in comparison to a dotplot or other visualizations, it cannot represent the number of observations there were. 
...
ggplot(data = nycflights %>%
  filter(dest == "LAX") , aes(x = dep_delay)) +
  geom_histogram()
```
...
The histogram of the arrival delays of the LAX flights is a unimodal distribution that is right-skewed. The fact that the histogram is right-skewed can also be seen by the fact that the median is -6, but the maximum is way larger at 319, while the minimum is at -62. In addition, this can be determined by the interquartile range, where the middle 30% of the data is 30.5. The variance, which shows the spread of the data is also very large, highlighting that the spread of the data is large.


```{r}
## Code for exercise 2 goes here! 
lax_flights %>%
  summarise (median_rr  = median(arr_delay),
            standard_rr = sd(arr_delay),
            variance_rr = var(arr_delay),
            iqr_rr      = IQR(arr_delay),
            minimum_rr  = min(arr_delay),
            maximum_rrn = max(arr_delay))
ggplot(data = nycflights %>%
  filter(dest == "LAX") , aes(x = arr_delay)) +
  geom_histogram()
...
From the visualization, you could identify more patterns that would be a lot harder to find in data summaries, as there are sometimes too much data to go through. In addition from visualizations, you can better gain a better grasp of the data as a whole and in different subsections, whereas data summaries mosty give specific facts about certain aspects of a data. 
