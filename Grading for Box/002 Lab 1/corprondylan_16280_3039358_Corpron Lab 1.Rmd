...
Carrier, Tailnum, Origin, Dest All are nominal
...
With the smaller binwidth we are able to see more precise data. There is a large peak right at 0 that doesn't appear on the larger binwidth histogram. By lumping all the data points together it also makes the count at each value higher which skews the data.
...
<!-- Your response goes below this line! -->
(a) Dotplot, Density plot and Boxplot
(b) It allows us to get a good picture of the spread of data we have. But, it is not a perfect construction of the data and that is primarily due to the binwidths not truly allowing for a proper representation because the shape of the distribution can change depending on what width you choose. The histogram isn't a very continuous display of data either, i.e. it looks a little janky with its peaks and dips.
...
Looking at the histogram we see a right skew to the graph itself and by looking at the summary statistics we can see that the mean is the best representation of the data in this case since the outliers didn't effect it enough to make it an unreasonable depiction of the data.
...
Rather than just being given the mean or median of the data, we get to actually see where all the data points lie which shows if certain months are more skewed than others. We also get to see the distribution of the data which gives us a picture of how the data summaries get their values.
