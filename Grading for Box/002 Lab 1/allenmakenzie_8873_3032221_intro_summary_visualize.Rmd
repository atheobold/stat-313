...
<!-- Your response goes below this line! -->
Yes, the categorical variables include carrier (nominal), tail number (nominal), flight (ordinal), origin (nominal), and destination (nominal).
...
Comparing these histograms, when the binwidth is larger, it encapsulates more data within each bin, which makes the bars much wider and easier to compare amongst each other to see the data density. However, a smaller binwidth reveals more details and more bars, which makes it easier to see the variation. 
...
<!-- Your response goes below this line! -->
a) Other ways to visualize the distribution of a numerical variable include a scatter plot, dotplot, or boxplot.
b) A histogram is useful for large data sets, and it provides a view of the data density, which the others do not. However, histograms do not provide a detailed view of the association between two variables, as a scatterplot would. Additionally, it is more difficult to see the unusual observations (outliers) in histograms, which is what a boxplot does well. 
...
The maximum arrival delays was 319, while the minimum was -62. There was a sample size of 1,583, and a mean number of arrival delays of 1.46. The histogram indicates the data density, which shows that the most dense area of the graph is at 0 arrival delays of LAX flights.
...
lax_flights %>%
  summarise(max_ad = max(arr_delay), min_ad = min(arr_delay), n = n(), mean_ad = mean(arr_delay))
...
ggplot(data = nycflights, aes(x = month, y = dep_delay)) + geom_point()
...
The visualization allows for easier identification of patterns and quicker understanding. Also, visualizations can show a larger amount of data on one screen, while a data summary might take up a lot of space and be difficult to decipher. 
