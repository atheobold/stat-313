...
<div style="color:blue">
What do you mean by n/a? That you think this question is not applicable? That there were no variables in the dataset that were categorical? I would encourage you to take a look at the data again! 
</div>
...
There is a small spike at the beginning of the 'bins = 15' histogram, whereas the other two have thicker bars, which makes the data simpler.

<div style="color:blue">
Nice observation about the size of the bars, but how does that relate to the features of the distribution that are easy / hard to see?  
</div>
...
<!-- Your response goes below this line! -->
a)Other visualizations include: boxplots, dotplots, density plots
b)Benefits of histogram in this case: Clear difference in data patterns, easu to conceptualize
  Weaknesses of histogram in this case: relies on bin value for clarity, vague at times

<div style="color:blue">
Great description of the strenghts and weaknesses of histograms! How do these qualities relate to the other plotting options?  
</div>
...
ggplot(data = nycflights %>% filter(dest == "LAX"), aes(x = dep_delay)) +
  geom_histogram()
```

<div style="color:blue">
You have the right idea for using the pipe operator, but you executed it in a different way than I was hoping! What you are doing is filtering the dataset **inside** the `ggplot()` function. What I was hoping you would do is filter the data **before** inputting it into the `ggplot()` function. This is how you see this process in the tutorials, so I am a bit unsure where you learned this approach. Please ask if you are not sure how you could have done it this way! 
</div>
...
-dest == "LAX
-summarise variables afterwards

<div style="color:blue">
Nice data summaries, but how did you decide what summaries to use? Where is your distribution of arrival delays? Where is your description tying these two pieces together? 
<div style="color:blue">
Overall: Redo
</div>
...
lax_delays <- nycflights %>%
  filter(dest == "LAX") %>%
  summarise(arrival_mean = mean(arr_delay),
            arrival_stdev = sd(arr_delay),
            arrival_var = var(arr_delay),
            arrival_max = max(arr_delay),
            arrival_min = min(arr_delay),
            n           = n())
glimpse(lax_delays)
...
dflights %>% arrange(desc(delay))

```

<div style="color:blue">
Nice work, the only thing I would change is to connect the `summarise()` step with the `arrange()` step with the %>% operator! 
</div>
...
ggplot(data = nycflights, aes(x = month)) +
  geom_histogram()

```

<div style="color:blue">
This plot returns an error, since the x-variable you gave it cannot be plotted. How do we plot the distribution of departure delays, faceted by month?
</div>
...
I immediately noticed that most of the columns have similar lengths, and that February
has the lowest value of all 12 months.
