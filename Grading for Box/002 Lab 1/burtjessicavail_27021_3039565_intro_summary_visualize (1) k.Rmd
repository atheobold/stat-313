...
The 'nycflights' data set has 32,735 rows and 16 columns.
...
<!-- Your response goes below this line! -->
Yes, there are categorical variables. They are year (integer), arr_delay (dbl), carrier (character), tailnum (character), origin (character), and dest (character).
...
The histogram with the higher binwidth number allows for a more accurate data grouping whereas the one with the smallest binwidth number, doesn't allow for the data to be properly grouped. However, the histogram with binwidth = 150 doesn't show much differentiation, so a big binwidth number isn't always good. Aspects of the data are seen in the one with a bigger binwidth number though. This is because it allows for a more detailed picture of what is happening in the data. 
...
<!-- Your response goes below this line! -->
a) We also could have used a boxplot, dotplot, or a density plot. 
b) A histogram is beneficial if you want to look at a summary of the data rather than the raw data. It is also a good graph to see the overall shape of the distribution (skewed/ symmetric/ modes). A boxplot is able to see all the observations (individual data points), which a histogram cannot do. 
...
The distribution of the arrival delays of the LAX flights is very skewed to the right.The mean being about 9.7 and the median being -1. 
...
From the visualizations, you can clearly see the distribution of delayed flights. You can also see (with a bigger binwidth number) how skewed the data is within each individual faceted histogram. 
