...
In this data set, there are 32,735 rows and 16 columns.
...
Yes, there are categorical variables in the dataset. They are year (integer), month (integer), day (integer), departure time (integer), arrival time(integer), carrier (character), tail number (character), flight number (integer), origin (character), destination (character). Categorical data is called a factor.
...
The histogram with the binwidth of 15, shows more in detail because if you look next to 0 there is a small bar going into the negative side of the graph. In the other two graphs, this sample is not seen. The histogram with a binwidth of 150 is thicker because it's clumping more results together, so you can see it goes into the negative part of the graph, but the bar makes it look like there were over 30,000. When in the 15 binwidth graph, the bar comes up to around 3,000. The first graph with the 30 binwidth still clumps results together but not as much as the 150 binwidth. You can still see it goes into the negative side of the graph, but it goes up to around 27,000.
...
We could have also used a dot plot, this would've been nice so we can see the raw data for each case. The only hard part for a dot plot is that there are a lot of observations, so the graph would have an overwhelming amount of dots. We could have also done a scatterplot, this would have shown us any trends in the data, like of there was a positive or negative correlation. The problem with scatterplots is that since there is a lot of cases in this study, there would also be an overwhelming amount of dots. It would look too cluttered. I think since there is a lot of raw data in this study a histogram worked the best. It also shows where the most amount of observations occurred. So looking at this graph, a person could see that a majority of flights had 0 departure delay.
...
For this graph, I used the minimum and maximum values. The minimum number of this dataset tell us that the earliest arrival time at LAX was 62 minutes early (I wish!). The maximum value tell us the longest arrival delay at LAX, which was 319 minutes. I think this shows us the best distribution. Originally, I tried the mean and the median, but the numbers I got were way different, I got 1.5 as the mean and -6 of the median. This does not show the distribution well because in the histogram there is a right skew and we have some outliers. Standard deviation and variance are also extremely sensitive to outliers, so using these would skew the data.
...
In visualizations, we are more easily able to notice patterns in the data. For example, in the histograms above, someone could notice that a majority of flights either left exactly on time or a little bit earlier. Someone could also notice that during the summer months (May-July) there were less flights leaving early/on time and more flights delayed, this also happened in December, which makes sense because of the holidays. If someone had a little but more time, they could look at the data summaries to get a specific number, but with visual graphs, it's more easy to spot general patterns rather than specific numbers.
