...
```{r}
ncbirths %>% 
  filter(is.na(mage) == FALSE, 
         is.na(weeks) == FALSE) %>% 
  summarize(correlation = cor(mage, weeks))
```
...
In __every__ statistical model we will encounter we require for the observations
in the data to be independent.  
