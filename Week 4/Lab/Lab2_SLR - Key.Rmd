---
title: Introduction to Linear Regression - Key
...
The `hfi` data has 1458 rows and 123 columns. 
...
The `hfi_small` dataset has 1458 total rows, but the `hfi_small` dataset with NAs 
removed only has 1378 rows. Thus, there are 80 total rows with missing values
...
Yes, it appears that there is a linear relationship between `pf_score` and 
`pf_expression_control`. Additionally, the relationship appears to get stronger 
as the values of `pf_expression_control` get larger, since the data become less 
spread out. 
...
hfi_small_clean %>% 
  ...
  geom_jitter() + 
  labs(x = "Personal Freedom -- Expression Control", 
...
Overall, I would feel comfortable using a linear model to predict the personal freedom score, as the relationship between the freedom expression score and the freedom score appears fairly linear. Indeed, there is a slight amount of curvature toward the larger values of expression control, where we see the freedom score tapering off. However, this is not overly concerning. 
...
The relationship between `pf_expression_control` and `pf_score` is postive and moderate to strong. As mentioned earlier, the relationship appears fairly linear, and there are only a few observations that do not directly follow the trend. 
...
I am quite bad at this, so the smallest sum of squared errors I got was 991.566. After reading everyone's methods and sum of squares, I think many of you are far better at finding a line of best fit by eye than I am! 
...
get_regression_table(hf_model)
```

The equation for the model is as follows: 

$$
\widehat{hf\_score} = 5.15 + 0.35 \cdot \text{pf_expression_control}
$$ 
This tells us that for every 1 unit increase in `pf_expression_control`, we expect the mean hf_score to increase by 0.35. This indicates that an increase in a country's freedom of expression is associated with an increase in the overall freedom of their citizens. 
...
__10.  If someone saw the least squares regression line and not the actual data, how would they predict a country's personal freedom score if the country had a 6.7 rating for `pf_expression_control`? Is this an overestimate or an underestimate?__ 
<!-- Your response goes here! Do not remove this line! -->

For a country with a personal freedom score of 6.7, we would predict they have a personal freedom score of around 7.9. At this point on the line it appears that there are more observations above the line than below the line. Hence, it is possible that an estimate of a personal freedom of 7.9 could be an overestimate. 
...
There doesn't appear to be any pattern in the residuals versus fitted plot. The points appear evenly distributed across the fitted values. Thus, I would conclude the condition of a linear relationship does not appear to be violated. 
...
The residuals appear fairly symmetric, however the center of the distribution appears to be slightly larger than 0. There is only one peak and both tails don't appear to be overly long. I would conclude that the condition of normal residuals does not appear to be violated. 
...
The variability in the residuals versus fitted plot appears to be fairly constant across the fitted values. There does not appear to be a trend where larger fitted values are associated with larger residuals or smaller fitted values associated with smaller residuals. Hence, I would conclude that the equal variance condition does not appear to be violated. 
